...
func TestEnsureLabelsNoopWithoutConfig(t *testing.T) {
	tr := &Tracker{}
	fields := map[string]interface{}{}
	tr.ensureLabels(fields)

	if _, exists := fields["labels"]; exists {
		t.Errorf("labels should not be set when no labels configured")
	}
}

func TestCreateIssueIncludesConfiguredLabels(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodPost {
			...
			w.Header().Set("Content-Type", "application/json")
			_ = json.NewEncoder(w).Encode(map[string]string{"id": "1", "key": "EXD-99", "self": r.URL.String()})
			return
		}
		// GET for re-fetch after create
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(issueResponse("EXD-99", "To Do"))
	}))
	defer srv.Close()
